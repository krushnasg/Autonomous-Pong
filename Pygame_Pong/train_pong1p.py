...
SCORE_MAX = 30
NUM_TEST = 5
NUM_TRAIN = 1000000 #takes ~100 minutes to train
...
# pkl_file = open('q_val_exp4.pkl', 'rb')
...
def reset_game() :
	...
	global paused_game
	global score
	ball_centre_y=(int((random.random()*100000)%(size[0]-20)))+6
	ball_centre_x=(int((random.random()*100000)%(size[0]-20)))+10
	#ball_centre_x = 50
	ball_direction=np.random.choice(set_actions)
	hit_bar_left=(int((random.random()*100000)%(size[0]-hit_bar_length-3)))+1
	if hit_bar_left % 2 == 0:
		hit_bar_left +=1
	time1=pygame.time.get_ticks()
	can_accel_left=False
...
def reset_game2() :
	...
	global paused_game
	global score
	ball_centre_y=size[1]//2
	ball_centre_x=(int((random.random()*100000)%(size[0]-20)))+10
	#ball_centre_x = 50
	ball_direction= np.random.choice(set_actions_game)
	hit_bar_left=(size[0]//2) - (hit_bar_length//2)
	if hit_bar_left % 2 == 0:
		hit_bar_left +=1
	time1=pygame.time.get_ticks()
	can_accel_left=False
...
def getLegalActions(state):
	if state[0]=='TERMINAL':
		...
		return ["RIGHT"]
	elif state[0]==size[0] - hit_bar_length - 1:
		return ["LEFT"]
	else:
		return ["LEFT","RIGHT"]
...
while num_training:
	# if posq%3000 == 0 and posq != 0:
	# 		print("posa " + str(posq//3000))
	if posq%1000000==0 and posq!= 0 :
		# print("op " + str(posq//3000))
		output = open('q_val.pkl', 'wb')
		pickle.dump(q_values, output)
		output.close()
	old_score=score
	...
	if(game_over==True):
		ball_hit_pos = getExpectedHitPos(ball_centre_x,ball_centre_y,ball_direction)
		next_state=('TERMINAL', ball_centre_x,ball_centre_y,ball_direction)
...
print("PRESS ANY KEY TO CONTINUE TO TESTING...")
while True :
	if wait_for_any_key() :
		break
